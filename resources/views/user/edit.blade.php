...
<div id="wrapper">
	<div class="main-content">
					...
					<h4 class="box-title">User @isset($user) - {{$user->id}} @endisset <a class="pull-right btn btn-primary btn-xs" href="{{route('users.list')}}">Back</a></h4>
							...
							<div class="form-group">
								...
								<select class="form-control" name="user_role">
									...
									@foreach($user_role as $role)
										<option @isset($user) @if($user->userRole->id == $role->id) selected @endif @endisset value="{{$role->status}}">{{$role->name}}</option>
									@endforeach
								</select>
							</div>
							<div class="form-group">
